...
The fixed path prognosis for reserve-dependent benefits -- with the path fixed in the active state -- is arguably the most common type of prognosis in the literature (e.g. \cite{NinnaReitzel}) and in the industry. It is the type pursued in \cite{Munk}, which forms the basis of the prognosis recommendation that the Danish pension and insurance industry association Forsikring \& Pension in 2018 communicated to its members (see \cite{Henstilling}). Consequently, it is the type of prognosis that most Danish life and pension companies offer their policy holders today.

In the language of this thesis, the fixed path prognoses for reserve-dependent benefits are given by

\begin{align*}
	\hat{b}^{F}(t) =& \econd[\textbf{W}(t-)\textbf{b}_{Z(t)}(t)]{Z(s) = z(s) : 0 \leq s \leq n}, \\
	\triangle \hat{B}^{F}(t) =& \econd[\textbf{W}(R-)\triangle \textbf{B}_{Z(R)}(R)]{Z(s) = z(s) : 0 \leq s \leq n}, \\
	\hat{b}_{\cdot k}^{F}(t) =& \econd[\textbf{W}(t-)\textbf{b}_{Z(t-)k}(t)]{ Z(s) = z(s) : 0 \leq s \leq n},
\end{align*}

for some $k \in \mathcal{J}$ and some deterministic function $z: [0,n] \to \mathcal{J}$. As mentioned, $z \equiv 0$ is by far the most common choice.

Since we condition on the entire state path, the mathematics are particularly tractable. The following proposition demonstrate this:

\begin{proposition}
	Given a deterministic path $z: [0,n] \to \mathcal{J}$, the fixed path prognoses are given by
	
	\begin{align*}
		\hat{b}^{F}(t) =& W_z(t-) b_{z(t-)}(t), \\
		\triangle \hat{B}^{F}(t) =& W_z(R-) \triangle B_{z(R-)}(R), \\
		\hat{b}_{\cdot k}^{F}(t) =& \1{z(t-) \neq k} \1{z(t) = k} W_z(t-) b_{z(t-) k}(t),
	\end{align*}

where $W_z$ is a deterministic function described by the differential equation

\begin{align*}
	\frac{\partial}{\partial t} W_z(t) = f_{z(t-)}(t,W_z(t-))
\end{align*}

for $t \in [0,n] \setminus \left( \{ R \} \cup \left( s : z(s) \neq z(s-) \right) \right)$. The initial value is $w_0$ and gluing conditions are

\begin{align*}
	W_z(R) =& W_z(R-) + h_{z(R-)}(R,W_z(R-)), \\
	W_z(u) =& W_z(u-) + g_{z(u-)z(u)}(u,W_z(u-)), \, \, u \in \{ s : z(s) \neq z(s-) \}.
\end{align*}
\end{proposition}
\begin{proof}
On the event
...
\begin{align}
	W(t) =& w_0 + \int_{(0,t)} dW(s) \nonumber \\
	=& w_0 + \int_{(0,t)} f_{z(s-)}(s,W(s-)) ds \nonumber \\
	&+ \int_{(0,t)} \sum_{k \in \mathcal{J} \atop k \neq Z(s-)} g_{z(s-)k}(s,W(s-)) dN_{k}(s) \nonumber \\
	&+ \int_{(0,t)} h_{z(s-)}(s,W(s-)) d \varepsilon_R(s) \nonumber \\
	=& w_0 + \int_{(0,t)} f_{z(s-)}(s,W(s-)) ds \nonumber \\
	&+ \sum_{s \in (0,t) \atop z(s) \neq z(s-)} g_{z(s-) z(s)}(s,W(s-)) \label{FirstKEK} \\
	&+ \1{t\geq R} h_{z(R-)}(R,W(R-)) \label{SecondKEK}
\end{align}

Since $f$ is continuous everywhere except $R$, it follows that $W(t)$ is differentiable on

\begin{align*}
	[0,n] \setminus \left( \{ R \} \cup \left( s : z(s) \neq z(s-) \right) \right),
\end{align*}

i.e. everywhere except at the time of retirement $R$ and the times where a jump occurs in the path $z$. The derivative is simply

\begin{align*}
	\frac{\partial}{\partial t} W(t) = f_{z(t-)}(t,W(t-))
\end{align*}

with initial value $w_0$. The gluing conditions can be deduced partly from the jumps of $z$ (equation (\ref{FirstKEK})) and partly from the lump sum payment at retirement $R$ (equation (\ref{SecondKEK})).

Denote the function described by the above scheme $W_z$. Then
...
as desired. Similar calculations yield the expressions for $\triangle \hat{B}^{F}(t)$ and $\hat{b}_{\cdot k}^{F}(t)$.
\end{proof}

Now, we can solve this scheme to obtain $W$ on the event $\left( Z(s) = z(s) : 0 \leq s \leq n \right)$. Denote it $W_z$. The prognosis is then simply obtained by

\begin{align*}
	z
\end{align*}

Differentiating XXX, we obtain
...
The restricted path prognoses for reserve-dependent benefits are the mathematically most challenging prognoses to obtain. The difficulty stems from the fact that -- unlike in the fixed path case -- we do not know the state of the policy holder except at the time of prognostication. We only know that she remains in the subset $\hat{\mathcal{J}}$.

The restricted path prognoses for reserve-dependent benefits are given by

\begin{align}
\hat{b}(t) =& \econd[W(t-) b_{Z(t-)}]{\{Z(s) \}_{0 \leq s < t} \in \hat{\mathcal{J}}} \\
\triangle  B(R) =& \econd[W(R-) \triangle B_{Z(R-)}]{\{Z(s) \}_{0 \leq s < R} \in \hat{\mathcal{J}}} \\
\hat{b}_{\cdot k}(t) =& \econd[W(t-) b_{Z(t-)k}]{\{Z(s) \}_{0 \leq s < t} \in \hat{\mathcal{J}}, Z(t) = k}
\end{align}
...
\begin{proposition} \label{MainResult}
	Let W be an $\mathcal{F}^{Z}$-adapted $n$-dimensional process with initial value $W(0)=w_0$ and dynamics as in equation (\ref{WDynamicsStart}).
...
In the preceding two sections, we have seen how to calculate fixed- and restricted path prognoses for a general process $W$ with affine dynamics in the sense (\ref{WDynamicsStart}). In the next, we shall see how a broad class of contract types often denoted \textit{With Profit} (as presented in \cite{Liv2Bog}) fit into this framework.

\newpage

\subsection{Example -- With Profit Insurance}
