...
\date{\today}
\usepackage{geometry}
\geometry{a4paper, left=25mm, right=25mm, bottom=30mm}
\usepackage{array}
\usepackage{bbm}
\usepackage{graphicx}
\usepackage{listings}
...
Having formally defined the three different prognoses, we close this chapter by exemplifying their use in three classical models:
...
\begin{example} \label{ExampleOne}
...
\begin{example} \label{3-tilstand}
...
In this model, most classical prognoses -- e.g. \cite{NinaReitzel} -- condition on the policy holder staying active all the way up to the time of payout $t$, e.g. the event $\left( Z(s) 0 : 0 \leq s < t \right)$. Instead we only assume that the policy holder does not die -- she might become disabled. So we let $\hat{\mathcal{J}} = \{ 0, 1 \}$, such that the time $t$ sojourn benefit prognosis becomes
...
Consider a multi-state model with policy holder options:
	...
	\begin{figure}[H]
		\begin{center}
			...
			\end{tikzpicture}
			\caption{Multi-state model including policyholder options}
...
\begin{example}
Consider a policy modelled as in Example \ref{3-tilstand}, i.e. the classical alive-disabled-dead model with reactivation:
...
\begin{itemize}
	\item \textbf{Baseline}: A best estimate scenario with premiums set at $\pi = 80$ t.DKK annually until retirement at $R=65$, a real return rate of 3\% per annum and transition intensities given as in (\cite{BuchartMoller}, Section 5):
